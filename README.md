...
1. Clone this `sodacl-workshop` repository.
2. Launch Docker in your local environment.
3. In Terminal, run the following command from the the root folder of the repo to set up the prepared testing environment:
`docker-compose up`
4. When the `database system is ready to accept connections`, open a new Terminal tab or window and from the root folder of the repo, run the following command to open a shell with the SodaCL files.
...
For this exercise, we're going to ask you to write some Soda Checks to discover bad-quality data in our demo database. 

#### About the demo data
The demo PostgreSQL database contains data from a fictitious, multinational manufacturing company called Adventure Works Cycles. It contains tables for customer information, employee information, sales territories, sales information, product categories, and more. To browse the contents of the database, use the DB connection details in the [configuration.yml](/sodacl/configuration.yml) file to connect to the database from your code editor.  

#### About Soda Checks

TODO reference a list of built-in checks?

#### Discover bad-quality data
1. Review [SodaCL: The Basics](#sodacl-the-basics) to get the context you need for this exercise.
2. Open the [checks.yml](/sodacl/checks.yml) file in the `sodacal` directory of this project. The Soda Check that already exists makes sure that there are between 20 and 100 rows in the table called `dim_account`. When you ran the Soda scan in the last step of the Set up instructions, this check passed with no failures, no warnings, no errors.
3. TODO
...
In the real world, you would connect Soda Core to a data source (Snowflake, Postgres, etc.), then define your Soda Checks for data quality in a YAML file, and use Soda Core to regularly run scans of your data to execute the checks. In the context of this program, we’ve simplified the experience to focus not on Soda Core as a tool, but on SodaCL, the language you use to define Soda Checks.

A **Soda Check** is a test that Soda Core performs when it scans a dataset in your data source. Technically, it is a Python expression that, during a scan, checks metrics to see if they match the parameters you defined for a measurement.
...
The following Soda Check makes sure that the `CUSTOMERS` dataset contains rows or, in other words, is not empty. In this example, count is the metric, and 0 is the measurement.
```yaml
checks for CUSTOMERS:
  - count > 0
```

You write Soda Checks using SodaCL’s built-in metrics. (Actually, you can go beyond the built-in metrics and write your own SQL queries, but that is an exercise for another day.) The example above is one of the simplest checks you can run on a single dataset, but SodaCL offers the built-in metrics that enable you to define checks for more complex situations.
...
For this program, the [checks.yml](/sodacl/checks.yml) file exists in the `sodacl` directory in this repo. You can edit the existing files to adjust the existing change and add others, or you can create a new YAML file to write your checks and run Soda scans, replacing `checks.yml` with your own file name in the soda scan command: 
